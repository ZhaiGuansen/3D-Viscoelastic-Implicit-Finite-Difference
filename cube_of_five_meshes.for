     ...
     + IMM2,IMS1,IMS2,IMS3,IM)
      ...
      IF(LABEL==2.OR.LABEL==3)THEN
          !含物理边界点,M1改为对应物理边界点,A设为1,M2设为-1
          !读取代码
     ...
     !! CALL DIA_ILU(AN,AM1,AM2,AS1,AS2,AS3,A,BN,BM1,BM2,
     !!+ BS1,BS2,BS3,B)
     ...
     + IAS1,IAS2,IAS3,M1M1,M1M2,M1S1,M1S2,M1S3,M2M1,M2M2,M2S1,M2S2,M2S3
      ...
      !READ(N,*,IOSTAT=IOS)IAN
      !ALLOCATE(IAM1(IAN(2)),IAM2(IAN(2)))
      !READ(N,*,IOSTAT=IOS)IAM1
      !READ(N,*,IOSTAT=IOS)IAM2
      !S=0
      !DO I=1,IAN(2)
      !    S=S+IAM2(I)
      !ENDDO
      !ALLOCATE(IAS1(S),IAS2(S),IAS3(S))
      !READ(N,*,IOSTAT=IOS)IAS1
      !READ(N,*,IOSTAT=IOS)IAS3
      !IAS2(1)=1
      !DO I=2,S
      !    IAS2(I)=IAS2(I-1)+IAS3(I-1)
      !ENDDO
      !S=IAS2(S)+IAS3(S)-1
      !ALLOCATE(IA(S))
      !READ(N,*,IOSTAT=IOS)IA
      ...
      !对B进行修改
      !LABEL取值0~7，二进制表示，(pml边界,物理边界,初值)
      IF(MOD(LABEL,2).EQ.1.AND.T<INI_T+1)THEN
          !初值
          LABEL=LABEL/2
          !文件名inin.dat,信息：ini_num初值点个数,ini_x初值点坐标,u,v,w初值 
     ...
     +     //TRIM(ADJUSTL(CT))//'.DAT'
      ...
      !!利用伪逆求初始解
      !CALL DOTSM(IAN,IAM1,IAM2,IAS1,IAS2,IAS3,IA,B,U(3,:),3*L**3)  
      U(3,:)=U(2,:)
      DO I=1,10
      ...
      WRITE(CTEMP,'(I6)')T
      FILENAME='.\OUTPUT\OUTPUT'//TRIM(ADJUSTL(CTEMP))//'.DAT'!OUTPUT+时间+.DAT
     ...
     + //TRIM(ADJUSTL(C2))//'.DAT'!OUTPUT+时间+.DAT
     ...
     + //TRIM(ADJUSTL(C2))//'.DAT'!OUTPUT+时间+.DAT
      ...
      DO NUM=1,3
          ...
          ENDDO
      ENDDO
      CLOSE(1,STATUS='DELETE')!t-2时刻文件读完即可删除
      CLOSE(2)
      ...
      DO NUM=1,3
          DO K=1,NZ
              DO J=1,NY
                  DO I=1,NX
                  READ(J,*)VALUE
                  !IF(VALUE.NE.0)WRITE(*,*)I,J,K,NUM,VALUE
